...
const MyRides = () => {
  ...
  const fetchRides = async (page = 1, status = null) => {
    try {
      setLoading(true);
      const response = await getMyRides(status, page, 10);
  ...
  const handleCancelRide = async (rideId, reason = '') => {
    ...
    try {
      ...
      const response = await cancelRide(rideId, reason);
  ...
  const canCancelRide = (ride) => {
    ...
    return ride.status === 'pending' || ride.status === 'confirmed' && rideDate > threeDaysFromNow;
  };
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mt-4 lg:mt-0 lg:ml-6">
                    ...
                    {canCancelRide(ride) && (
                      <button
                        ...
                        className="flex items-center justify-center px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:opacity-50"
                      >
